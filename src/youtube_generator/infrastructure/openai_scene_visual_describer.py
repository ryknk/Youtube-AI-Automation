...
class OpenAISceneVisualDescriber:
    ...
    def _instructions() -> str:
        return (
            ...
            "与えられた日本語のナレーション文は、1本の動画を構成する一連の場面を順番に表します。"
            "各場面の説明を書く際は、前後の場面の文脈（登場人物・場所・時間帯・雰囲気の流れ）を"
            "踏まえ、動画全体として自然につながる描写にしてください。ただし各説明はあくまで"
            "その場面固有の状況・雰囲気・登場人物の様子を、1〜2文の英語で視覚的に説明するものとし、"
            "前後の場面の内容をその説明文へ書き込んだり、複数の場面を1つの説明にまとめたりしない"
            "でください。各説明は必ず英語の平文のみとし、日本語や引用符、Markdown、見出しは含め"
            "ないでください。カンマ区切りの単語・タグの羅列ではなく、主語と動詞を備えた具体的で"
            "明確な文章にしてください。ナレーション文そのものの翻訳や引用ではなく、画面に映る"
            "情景の描写にしてください。各説明は必ず単一の場所・状況のみを描写し、屋内と屋外など"
            "異なる複数の場所を1つの画像内に混在させる描写にしないでください。ナレーション文が"
            "考察・要約・まとめのような抽象的な内容で具体的な場面を示していない場合でも、"
            "その内容を象徴する具体的な場所を1つだけ選び、その場所のみを描写してください。"
            "字幕・テロップ・文字・ロゴ・透かしなど、画面内に文字として"
            "表示される要素には一切言及しないでください。"
