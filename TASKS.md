...
`src/youtube_generator/jobs/pipeline.py`の`ExistingPipelineRunner`は、各工程ごとに`main.py`を`subprocess.run`で個別起動し、ステージ間の受け渡しを`output/`以下を再帰的に`rglob`して「新しく作られたディレクトリ」をmtime最大で特定する仕組み（`_new_script_dir`）に依存している。同時実行や他プロセスの出力生成と衝突するリスクがある。

- 対応方針: 未確定。改修する場合は影響範囲（ジョブキュー全体の挙動）が大きいため、着手前に方針をユーザーと相談する。
